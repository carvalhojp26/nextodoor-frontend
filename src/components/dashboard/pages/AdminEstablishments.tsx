...
interface EstabelecimentoFromAPI {
  ...
  // imagemEstabelecimento?: string; // REMOVIDO
}
...
export const AdminEstablishments = () => {
  ...
  // --- useEffects e Funções Fetch (mantidos como antes, com pequenos ajustes nos logs se necessário) ---
  useEffect(() => {
    fetchNeighborhoods();
    fetchAddresses();
  }, []);
      ...
      <div className="flex flex-col flex-1 overflow-y-auto">
        ...
        <main className="p-6 xl:p-8">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
            ...
            <button
              className="btn-primary mt-4 sm:mt-0" // Usando sua classe global .btn-primary
          ...
          <div className="mb-6 bg-white p-4 rounded-lg shadow border border-gray-200">
            ...
            <select
              ...
              className="form-select mt-1" // Usando sua classe .form-select
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
              {estabelecimentos.map(est => (
                <div key={est.idEstabelecimento} className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col overflow-hidden group">
                  {/* Imagem do Estabelecimento Removida */}
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    <div className="mt-5 pt-4 border-t border-gray-100">
                         <button
                            onClick={() => handleOpenDetailsModal(est)} // Ação principal do card pode ser ver detalhes
                            className="text-sm text-blue-600 hover:text-blue-700 font-medium w-full text-left py-1"
                        >
                            Ver Detalhes →
                        </button>
                        {/* TODO: Adicionar botões de Editar/Excluir aqui se necessário, 
                                  usando suas classes .btn-edit-small e .btn-danger-small */}
                    </div>
                  </div>
                </div>
              ))}
          ...
          {totalPages > 1 && (
            <div className="mt-10 flex justify-center items-center space-x-2">
                <button
                    ...
                    className="btn-secondary p-2 rounded-full disabled:opacity-50" // Usando .btn-secondary e tornando-o redondo
                    ...
                    typeof page === 'number' ? (
                        <button
                            ...
                            className={`px-3.5 py-1.5 rounded-md text-sm font-medium transition-colors
                                ${currentPage === page
                                ? 'btn-primary' // Página atual usa .btn-primary
